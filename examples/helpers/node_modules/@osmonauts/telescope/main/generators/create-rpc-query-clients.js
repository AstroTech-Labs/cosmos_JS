"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.plugin = void 0;

var _imports = require("../imports");

var _ast = require("@osmonauts/ast");

var _protoParser = require("@osmonauts/proto-parser");

var _parse = require("../parse");

var plugin = function plugin(builder, bundler) {
  // if (!builder.options.rpcClients.enabled) {
  //     return;
  // }
  var clients = bundler.contexts.map(function (c) {
    var _proto$Query, _proto$Service;

    var enabled = c.proto.pluginValue('rpcClients.enabled');
    if (!enabled) return;
    var ctx = bundler.getFreshContext(c); // get mutations, services

    (0, _parse.parse)(ctx);
    var proto = (0, _protoParser.getNestedProto)(c.ref.traversed);
    var name, getImportsFrom;

    if ((!(proto !== null && proto !== void 0 && proto.Query) || ((_proto$Query = proto.Query) === null || _proto$Query === void 0 ? void 0 : _proto$Query.type) !== 'Service') && (!(proto !== null && proto !== void 0 && proto.Service) || ((_proto$Service = proto.Service) === null || _proto$Service === void 0 ? void 0 : _proto$Service.type) !== 'Service')) {
      return;
    }

    if (proto.Query) {
      name = 'query';
      getImportsFrom = ctx.queries;
    } else if (proto.Service) {
      name = 'svc';
      getImportsFrom = ctx.services;
    }

    var localname = bundler.getLocalFilename(c.ref, "rpc.".concat(name));
    var filename = bundler.getFilename(localname);
    var asts = [];

    if (proto.Query) {
      asts.push((0, _ast.createRpcClientInterface)(ctx.generic, proto.Query));
      asts.push((0, _ast.createRpcClientClass)(ctx.generic, proto.Query));
    }

    if (proto.Service) {
      asts.push((0, _ast.createRpcClientInterface)(ctx.generic, proto.Service));
      asts.push((0, _ast.createRpcClientClass)(ctx.generic, proto.Service));
    } ////////


    var serviceImports = (0, _imports.getDepsFromQueries)(getImportsFrom, localname); // TODO we do NOT need all imports...

    var imports = (0, _imports.buildAllImports)(ctx, serviceImports, localname);
    var prog = [].concat(imports).concat(ctx.body).concat(asts);
    bundler.writeAst(prog, filename);
    bundler.addToBundle(c, localname);
    return {
      "package": c.ref.proto["package"],
      localname: localname,
      filename: filename
    };
  }).filter(Boolean);
  bundler.addRPCQueryClients(clients);
};

exports.plugin = plugin;
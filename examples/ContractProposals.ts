...
export async function ContractTest() {
  let proposalId = 0;
  let json = {};
  const val1 = await CosmosChainClient.init(
  ...
  const test1 = await CosmosChainClient.init(
  ...
  const test2 = await CosmosChainClient.init(
  ...
  const test3 = await CosmosChainClient.init(

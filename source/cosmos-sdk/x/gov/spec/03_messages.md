...
- Transfer `InitialDeposit` from the `Proposer` to the governance
  `ModuleAccount`
...
bonded Atom holders are able to send `MsgVote` transactions to cast their vote
on the proposal.
...
_Note: Gas cost for this message has to take into account the future tallying of
the vote in EndBlocker_

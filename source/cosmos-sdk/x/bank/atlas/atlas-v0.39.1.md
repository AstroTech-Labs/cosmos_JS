...
The `x/bank` module is responsible for handling multi-asset coin transfers
...
transfers while ensuring critical network functionality is operating as
expected.
...
Note, the `x/bank` module does not natively support constructing a
`MsgMultiSend` message. This type of message must be constructed manually, but
it may be signed and broadcasted via the CLI.

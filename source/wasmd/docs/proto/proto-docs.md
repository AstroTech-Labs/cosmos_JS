...
MsgInstantiateContract create a new smart contract instance for the given code
id.
...
Note: skipping 3 as this was previously used for unneeded run_as | | `contract`
| [string](#string) | | Contract is the address of the smart contract | |
`code_id` | [uint64](#uint64) | | CodeID references the new WASM code | | `msg`
| [bytes](#bytes) | | Msg json encoded message to be passed to the contract on
migration |
...
UnpinCodesProposal gov proposal content type to unpin a set of code ids in the
wasmvm cache.
...
UpdateInstantiateConfigProposal gov proposal content type to update instantiate
config to a set of code ids.
...
QueryAllContractStateRequest is the request type for the Query/AllContractState
RPC method
...
QueryContractHistoryResponse is the response type for the Query/ContractHistory
RPC method
...
QueryContractsByCodeResponse is the response type for the Query/ContractsByCode
RPC method
...
QueryRawContractStateRequest is the request type for the Query/RawContractState
RPC method
